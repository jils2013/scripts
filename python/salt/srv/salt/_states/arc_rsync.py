...
def synchronized(name, source,
    ...
    source
        Directory source starting with.
        We will remove single top level directory(extracted from an archive).
    ...
    if not os.path.exists(name) and not force and not prepare:
        ...
        result = __salt__['rsync.rsync'](_get_root(source), name, delete=delete,
