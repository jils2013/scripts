...
#Bearer realm...
user={
	'http://youregistry.com/v2/token':['user','password'],
}

#get running images from kubernetes deployments
def getrunimgs(url):
	images=[]
	for i in json.loads(urllib2.urlopen('http://'+url+'/apis/extensions/v1beta1/deployments').read())['items']:
...
def getrepositories(host):
	return apirequest('http://'+host+'/v2/_catalog','')['repositories']
...
def deleteimagewithtag(img):
	digest=apirequest('http://'+img[0]+'/v2/'+img[1]+'/manifests/'+img[2],'HEAD')
	if type(digest)==type(''):
		return False
	delete=apirequest('http://'+img[0]+'/v2/'+img[1]+'/manifests/'+digest['Docker-Content-Digest'],'DELETE')
	if delete != 202:
		return False
	return True
