...
function _M.decrypt_with_public_key(self, str)
    local rsa = C.EVP_PKEY_get1_RSA(self.pkey)
    if not rsa then
        return nil, "not inited for decrypt"
    end
...
function _M.x509_get_pubkey(self, str)
    local bio_method = C.BIO_s_mem()
    local bio = C.BIO_new(bio_method)
    ffi_gc(bio, C.BIO_vfree)

    local len = C.BIO_puts(bio, str)
    if len < 0 then
        return nil, ssl_err()
    end
    ...
    local pubkey = C.X509_get_pubkey(x509)
    if pubkey == nil then
        return nil, ssl_err()
    end
    ffi.gc(pubkey, C.EVP_PKEY_free)

    local pub_key_bio = C.BIO_new(C.BIO_s_mem())
    ffi_gc(pub_key_bio, C.BIO_vfree)
    if C.PEM_write_bio_PUBKEY(pub_key_bio, pubkey) ~= 1 then
        return nil, ssl_err()
    end 
